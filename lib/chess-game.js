...
class ChessGame extends EventEmitter {
  ...
  async play() {
    let act = this.checkInterrupt();

    if (act === TAKE_BACK_ACT) {
      this.clearInterrupt();
      return this.waitForTakeBack();
    } else if (act === RESET_ACT) {
      this.clearInterrupt();
    ...
    act = await this.waitPlayerTurn(color);
    this._pendingState = false;
  ...
  takeBack() {
    if (this._pendingState !== "wait-board-ready" && this._pendingState !== "wait-board-sync") {
