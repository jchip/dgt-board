...
class ChessGame extends EventEmitter {
  ...
  waitForBoardReady(wantRaw, event, readyCb) {
    ...
    if (!waitReady()) {
      this._logger.log("wating for board positions to be ready");
  ...
  makeVisualMoveAscii(beforeAscii, afterAscii) {
    let visualMoveAscii = "";
    const before = beforeAscii.split("\n"); //beforeBoard.split("\n");
    const after = afterAscii.split("\n"); // this._chess.ascii().split("\n");
  ...
  takeBack() {
    ...
    this.waitForBoardReady(this._chess.toString(), event, () => {
      ...
      this._board.resetTo(this._board.toString());
