...
class ChessGame extends EventEmitter {
  ...
  waitForBoardReady(wantRaw, event, readyCb) {
    const wantAscii = utils.rawToAscii(wantRaw.split("")).join("\n");
    const waitReady = () => {
      ...
      if (boardRaw === wantRaw) {
        ...
        const boardAscii = utils.rawToAscii(boardRaw.split("")).join("\n");
  ...
  takeBack() {
    ...
    const event = "take-back-board-ready";
    this._chess.undo();
    this._chess.undo();
  ...
  checkTakeback(color, move) {
    ...
    if (last.from === to && last.to === from && color[0] === last.color[0]) {
      ...
      Object.keys(this._players).forEach(c => {
        this._players[c].pause();
        this._players[c].takeBack();
      });
    }
  }
}
